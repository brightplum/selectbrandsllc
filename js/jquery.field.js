...
(function($){
	...
	$.fn.createCheckboxRange = function(callback){
		var iLastSelection = 0, self = this, bCallback = (typeof callback == "function");
		...
		if( bCallback ) 
			this
				.bind("createCheckboxRange", callback)
				.trigger("createCheckboxRange");
		...
		return this.each(
			function (lc){
				...
				var checkboxClicked = function (e){
					...
					$(this).trigger("createCheckboxRange", [bSetChecked]);
					...
					if( bCallback )
						...
						items.trigger("createCheckboxRange", [bSetChecked]);
				};
